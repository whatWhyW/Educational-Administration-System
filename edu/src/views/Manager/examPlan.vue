<template>
  <div>
    <!-- <div style="height: 90px;">
      <span class='font'>欢迎进入本校考试安排模块</span>
      <p class='text'>考试是测验学生水平的关键，请慎重安排考试，以帮助学员更好地成长</p>
    </div> -->
    <el-row>
      ...
      <div class="searchbox" style="display: inline-block; float: right;">
        ...
        <el-button type="warning" @click="sortUp()" style='background-color: #FBF0EE; color: #E6A23C'>考试时间升序</el-button>
        <el-button type="warning" @click="sortDown()"  style='background-color: #FBF0EE; color: #E6A23C'>考试时间降序</el-button>
      ...
      <el-dialog title="请填写考试计划" :visible.sync="dialogFormVisible" style="height: 100%">
        <el-form :model="form" :rules="rules" ref="form">
          ...
          <el-form-item label="成绩分配" prop="score">
           ...
           <el-form-item label="考试类型" prop="catagory">
	          <el-select style='width: 100%; position: absolute; left: 136px' v-model="form.catagory" placeholder="请选择考试类型">
	            <el-option label="java" value="java" autocomplete="off"></el-option>
	            <el-option label="web" value="web" autocomplete="off"></el-option>
              <el-option label="测试" value="测试" autocomplete="off"></el-option>
              <el-option label="UI" value="UI" autocomplete="off"></el-option>
	          </el-select>
	        </el-form-item>
          ...
          <!-- <el-form-item label="适应阶段" prop="courseGrade">
	          <el-input v-model="courseGrade" autocomplete="off"></el-input>
	        </el-form-item> -->
        </el-form>
  ...
  methods: {
    ...
    sendRequest(url, opt = {}) {
      ...
      this.$http.post(url, opt).then(function(res) {
          // console.log(res)
          // 将数据存储起来
          if (url === '/manager/addExamPlan') {
            // console.log("添加用户信息")
            _this.tableData.push(res.data)
          } else if (url === '/manager/editExamPlan') {
            // console.log("编辑用户信息")
            _this.getAllData()
          } else if (url === '/manager/delExamPlan') {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if (that.isEdit) {
            ...
            opt._id = that.editId
            // 修改
            that.sendRequest('/manager/editExamPlan', opt)
            // this.getAllData()
          } else {
            // 新增
            that.sendRequest('/manager/addExamPlan', that.form)
            // this.getAllData()
          }
          // that.getAllData()
    ...
    getAllData() {
      var _this = this
      this.$http.get('/manager/examPlanAll').then(function(res) {
          console.log(res)
    ...
    editData(index) {
      ...
      this.form.catagory = this.tableData[index].catagory
    ...
    delData() {
      this.sendRequest('/manager/delExamPlan', {id: this.delId})
