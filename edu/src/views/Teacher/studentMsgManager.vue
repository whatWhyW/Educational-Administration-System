  ...
  methods: {
    sendRequest(url, opt={}) {
      ...
      this.$http.post(url, opt).then(function (res) {
        // console.log(res)
         // 将数据存储起来
        if(url === '/teacher/addStudent') {
          // console.log("添加用户信息")
          // console.log(res)
          _this.tableData.push(res.data)
        } else if(url === '/teacher/editStudent') {
          // console.log("编辑用户信息")
          _this.tableData = res.data.data
        } else if(url === '/teacher/delStudent') {
          // console.log("编辑用户信息")
          _this.tableData = res.data.data
        }
      })
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if(that.isEdit) {
            ...
            opt._id = that.editId
            // 修改
              that.sendRequest('/teacher/editStudent',opt)
            }else {
              // 新增
              that.sendRequest('/teacher/addStudent',that.form)
            }
    ...
    getAllData() {
      ...
      this.$http.post('/teacher/findMsg', {teacherName: JSON.parse(window.localStorage.teaInfo).username}).then(function (res) {
        ...
        _this.$http.post('/teacher/studentsAll', {className: res.data.className}).then(function (res) {
          console.log(res)
          if(res.data.length === 0) {
            alert('Sorry,该系统还没有您的班级学员，请联系班主任');
            return;
          }
      ...
      // this.$http.get('/teacher/studentsAll').then(function (res) {
      //   console.log(res)
      //   _this.tableData = res.data
      // })
      // .catch(function (error) {
    ...
    searchData() {
      ...
      var result = _this.tableData.filter(function(value, index, arr) {
        ...
        return document.getElementsByClassName('sc')[0].value == value.name || 
